...
describe('Audit Logger', () => {
  ...
  it('should log an audit entry to the log file', done => {
    logAudit('testAction', 'testUser', { foo: 'bar' });
    setTimeout(() => {
      const content = fs.readFileSync(logFile, 'utf8');
      expect(content).toMatch(/"action":"testAction"/);
      expect(content).toMatch(/"userId":"testUser"/js
